class Favorite < ActiveRecord::Base
  ...
  def self.my_favorites_projects_to_a(user)
    my_favorites_projects(user).to_a
  end

  def self.my_favorite_selected_project(user)
    my_favorites_projects_to_a(user).reject { |favorite_project| favorite_project.project.blank? }
  end
